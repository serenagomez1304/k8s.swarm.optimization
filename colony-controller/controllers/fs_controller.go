...
func (r *ColonyReconciler) foodsourceController(ctx context.Context, reqLogger logr.Logger, instance *abcoptimizerv1.Colony, req ctrl.Request) (ctrl.Result, error) {
	...
	reqLogger.Info("Foodsource instance status: " + fmt.Sprint(instance.Status))
...
func reconcileWithEmployeeInProgress(instance *abcoptimizerv1.Colony, reqLogger logr.Logger) (ctrl.Result, error) {
	...
	for bee, value := range instance.Status.EmployeeBees {
		...
		case "Waiting":
			...
			reqLogger.Info("reconcileWithEmployeeInProgress: waiting for onlooker, reserving")
			newFoodsource.ReservedBy = bee
		case "Running":
			// occupy
			reqLogger.Info("reconcileWithEmployeeInProgress: running, occupying")
			newFoodsource.OccupiedBy = bee
			newFoodsource.ReservedBy = ""
		case "Done":
			// vacate
			reqLogger.Info("reconcileWithEmployeeInProgress: done, vacate")
			good, err := compareFitness(value.ObjectiveFunction, instance.Status.FoodSources[bee].ObjectiveFunction, reqLogger)
			if err != nil {
				return ctrl.Result{}, err
			}
			if good {
				...
				newFoodsource.OccupiedBy = ""
				newFoodsource.Foodsource = instance.Status.FoodSources[bee].Foodsource
				newFoodsource.TrialCount = value.FoodsourceTrialCount + 1
				newFoodsource.ObjectiveFunction = instance.Status.FoodSources[bee].ObjectiveFunction
			}
		}
...
func reconcileWithOnlookerInProgress(instance *abcoptimizerv1.Colony, reqLogger logr.Logger) (ctrl.Result, error) {
	...
	for bee, value := range instance.Status.OnlookerBees {
		...
		case "Waiting":
			...
			reqLogger.Info("reconcileWithOnlookerInProgress: waiting for onlooker, reserving")
			newFoodsource.ReservedBy = bee
		case "Running":
			// occupy
			reqLogger.Info("reconcileWithOnlookerInProgress: running, occupying")
			newFoodsource.OccupiedBy = bee
			newFoodsource.ReservedBy = ""
		case "Done":
			// vacate
			reqLogger.Info("reconcileWithOnlookerInProgress: done, vacate")
			good, err := compareFitness(value.ObjectiveFunction, instance.Status.FoodSources[bee].ObjectiveFunction, reqLogger)
			if err != nil {
				return ctrl.Result{}, err
			}
